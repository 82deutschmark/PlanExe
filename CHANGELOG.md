...
- Adopted the shared WebSocket resolver in both monitoring dashboards so reverse-proxy deployments stream updates without duplicating endpoint glue code.
- Escaped plan identifiers within the shared WebSocket builder to prevent malformed socket paths when IDs contain spaces or reserved characters.
