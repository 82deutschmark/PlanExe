...
import type { WebSocketLLMStreamMessage, WebSocketMessage } from '@/lib/api/fastapi-client';
...
export const Terminal: React.FC<TerminalProps> = ({
  ...
  const detectLogLevel = useCallback((text: string): LogLine['level'] => {
  ...
  useEffect(() => {
      ...
      switch (message.type) {
        ...
        case 'log':
          if ('message' in message && message.message) {
            addLog(message.message, detectLogLevel(message.message));
          }
          break;
        ...
        case 'status': {
          const statusMsg = message as WebSocketMessage & { status?: string; message?: string };
          if (statusMsg.status === 'completed') {
            ...
            setStatus('failed');
            addLog(`❌ Pipeline failed: ${statusMsg.message || 'Unknown error'}`, 'error');
            if (onError) onError(statusMsg.message || 'Pipeline execution failed');
          } else if (statusMsg.status === 'running') {
            setStatus('running');
            if (statusMsg.message) {
              addLog(statusMsg.message, 'info');
            }
          }
          break;
        }
        ...
        default: {
          ...
          const rawMsg = message as { message?: string; text?: string };
          const logText = rawMsg.message || rawMsg.text || JSON.stringify(message);
    ...
    const handleClose = () => {
      setWsConnected(false);
      addLog('🔌 WebSocket connection closed. Falling back to polling...', 'warn');
    ...
    client
      ...
      .catch(() => {
        addLog('❌ WebSocket connection failed. Using polling fallback.', 'error');
        startPollingFallback();
      });
  ...
  }, [planId, addLog, handleLlmStreamMessage, detectLogLevel, onComplete, onError, startPollingFallback, stopPolling]);
