...
test('encodes plan identifiers to keep the WebSocket path RFC3986 compliant', () => {
  const url = buildWebSocketUrl(
    'plan with spaces/and/slashes',
  ...
  assert.equal(
    url,
    'ws://localhost:8080/api/ws/plans/plan%20with%20spaces%2Fand%2Fslashes/progress'
  );
});
